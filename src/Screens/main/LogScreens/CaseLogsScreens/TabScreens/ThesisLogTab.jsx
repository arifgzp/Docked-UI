...
const ThesisLogTab = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const fetchThesisLog = store.fetchThesisCaseByUser(
  ...
  useEffect(() => {
    const fetchThesisLogData = async () => {
      try {
        const fetchThesisLogData = store.fetchThesisLogByUser(
          appStoreInstance.UserName
        );
  ...
  const handleOnConfirmDeleteLog = async () => {
    try {
      ...
      const updateUserQuery = store.updateUser(appStoreInstance.UserId, {
        remove: { [`${modifiedTypename}s`]: { id: thesisCaseToBeDeleted.id } },
      });
      ...
      if (data) {
        ...
        if (query) {
          console.log(
            "Query from query delete with academicLogToBeDeleted ID",
            thesisCaseToBeDeleted.id,
            query
          );
          setQuery(query);
        ...
        <VStack width="$100%" pb="$3">
          ...
          >
            ...
            <HStack alignItems="center">
              ...
              <Button
                bg="transparent"
                onPress={() => handleEditACase(card?.id)}
                height={30}
                borderRadius="$full"
                size="xs"
              >
  ...
  const handleOnAddCase = () => {
    navigation.navigate("Create New Case For Thesis", {
      fieldsToGetFrom: "Thesis",
    });
  };
          ...
          <VStack width={"$100%"} alignItems="center" paddingBottom={"$15%"}>
            ...
            {store.ThesisLogList[0] && (
              <HStack width={"$100%"} justifyContent="space-between">
                <Box>
                  <Button
                    onPress={handleEditThesisLog.bind(null, thesisLog[0]?.id)}
                    size="sm"
                    variant="link"
                  >
                    <HStack space="sm" alignItems="center">
            ...
            {cardDetails.length > 0
              ? cardDetails.map((card, index) => (
                  <CardToRender card={card} index={index} />
                ))
              : !store.ThesisLogList[0] && (
                  <Button
