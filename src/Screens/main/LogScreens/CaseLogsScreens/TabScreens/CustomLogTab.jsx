...
const CustomLogTab = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const fetchCustomCase = store.fetchCustomCaseByUser(
          appStoreInstance.UserName
        );
  ...
  useEffect(() => {
    const fetchCustomLogData = async () => {
      try {
        const fetchCustomLogData = store.fetchCustomLogByUser(
          appStoreInstance.UserName
        );
  ...
  useEffect(() => {
    if (filteredCustomLogid) {
      const filteredCards = cardDetails.filter(
        (card) => card.customLogIdReference === filteredCustomLogid
      );
  ...
  const handleSelectCustomLogFilter = (customLog) => {
    setFilteredCustomLog(
      customLog.customName === filteredCustomLog
        ? undefined
        : customLog.customName
    );
    setFilteredCustomLogid(
      customLog.id === filteredCustomLogid ? undefined : customLog.id
    );
  ...
  const handleOnConfirmDeleteLog = async () => {
    try {
      ...
      if (typename === "CustomLog") {
        ...
        const relatedCases = store.CustomCaseList.filter(
          (caseItem) =>
            caseItem.customLogIdReference === customCaseToBeDeleted.id
        );
        ...
        if (relatedCases.length > 0) {
          ...
          if (casesRemovalResult) {
            ...
            const deleteCustomCasesQuery =
              store.deleteCustomCase(relatedCaseIds);
        ...
        if (data) {
          ...
          if (!rootStoreAPIRef) {
            throw new Error(
              `API method ${rootStoreAPIName} not found in root store`
            );
          }
      ...
      <VStack width="$100%" pb="$3">
        ...
        >
          ...
          <HStack alignItems="center">
            ...
            <Button
              bg="transparent"
              onPress={() => handleButtonPress(card?.id)}
              height={30}
              borderRadius="$full"
              size="xs"
            >
          ...
          <VStack width={"$100%"} alignItems="center" paddingBottom={"$15%"}>
                ...
                >
                  ...
                  {customLogs.map((customLog, index) => {
                    return (
                      <Pressable
                        key={customLog.id}
                        onPress={handleSelectCustomLogFilter.bind(
                          null,
                          customLog
                        )}
                        borderRadius={16}
                        ...
                        borderColor="#1e1e1e"
                        backgroundColor={
                          filteredCustomLog === customLog.customName
                            ? "#367B71"
                            : "#E6E3DB"
                        }
                        elevation={4}
            ...
            {filteredCustomLog !== undefined ? (
              <HStack width={"$100%"} justifyContent="space-between">
                <Box>
                  {/* <Button
                    onPress={handleOnPressEditCustomLogs}
                    size="sm"
                    variant="link"
                  >
                    <HStack space="sm" alignItems="center">
                  ...
                  >
                    <MenuItem
                      key="Community"
                      textValue="Community"
                      onPress={handleEditCustomLog.bind(
                        null,
                        filteredCustomLogid
                      )}
                    >
                      <MenuItemLabel size="sm" color="$white">
                        Edit Case
                      </MenuItemLabel>
                    </MenuItem>
                    <MenuItem
                ...
                <Box>
                  <Button
                    onPress={handleOnAddingANewCase.bind(
                      null,
                      filteredCustomLogid
                    )}
                    size="sm"
            ...
            >
              <ModalBackdrop />
              <ModalContent bg="#E6E3DB">
                <ModalHeader>
                  <Heading color="" size="md" className="text-typography-950">
                    {customCaseToBeDeleted?.__typename === "CustomLog"
                      ? `Delete "${customCaseToBeDeleted?.customName}"`
                ...
                <ModalBody>
                  <Text size="sm" className="text-typography-500">
                    {customCaseToBeDeleted?.__typename === "CustomLog"
                      ? `All your log entries in "${customCaseToBeDeleted?.customName}" this will be permanently deleted.   `
                ...
                <ModalFooter>
                  <HStack w="$60%" justifyContent="space-between">
                    ...
                    <Button
                      ...
                      variant="primary"
                      bg="#CC3F0C"
                      onPress={handleOnConfirmDeleteLog}
                      borderWidth={1}
                      borderColor="#1e1e1e"
                    >
            ...
            <Modal
              isOpen={showCustomCasesModal}
              onClose={() => setCustomCasesModal(false)}
              size="lg"
            >
              <ModalBackdrop />
              <ModalContent pb="$4">
            ...
            <Modal
              isOpen={showCustomLogsModal}
              onClose={() => setCustomLogsModal(false)}
              size="lg"
            >
              <ModalBackdrop />
              <ModalContent pb="$4">
                    ...
                    <VStack space="md">
                      ...
                      >
                        ...
                        <Button
                          ...
                          size="xs"
                          onPress={() =>
                            handleDeleteLog(filteredWholeCustomLog)
                          }
                          ref={ref}
                          alignItems="center"
                        >
